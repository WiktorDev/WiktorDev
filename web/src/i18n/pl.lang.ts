export default function() {
  return {
    ...
    projects: {
      ...
      'tuvio': {
        description: 'Platforma edukacyjna łącząca uczniów z korepetytorami, umożliwiająca wyszukiwanie ofert, rezerwowanie lekcji, obsługę płatności, komunikację oraz zarządzanie procesem nauki w jednym systemie.',
        highlight: 'Modularna platforma edukacyjna z marketplace korepetytorów, rezerwacją lekcji, płatnościami, komunikacją realtime oraz architekturą przygotowaną do dalszego wydzielania mikroserwisów.',
        overview: 'Tuvio to platforma edukacyjna zaprojektowana jako centralne miejsce łączące uczniów i korepetytorów. System umożliwia tworzenie profili korepetytorów, publikowanie ofert lekcji, wyszukiwanie odpowiednich nauczycieli, rezerwowanie terminów, obsługę płatności oraz prowadzenie komunikacji pomiędzy uczestnikami. Istotnym elementem platformy jest system zarządzania lekcjami, obejmujący ich planowanie, zmianę terminów, statusy oraz historię realizacji. Backend został zaprojektowany w sposób modularny, dzięki czemu poszczególne obszary domenowe mogą być w przyszłości niezależnie rozwijane i wydzielane do osobnych usług.',
        problem: 'Uczniowie często muszą korzystać z wielu niezależnych narzędzi do znalezienia korepetytora, ustalenia terminu, komunikacji, płatności oraz zarządzania kolejnymi lekcjami. Z perspektywy korepetytora problemem jest natomiast konieczność samodzielnego organizowania kalendarza, ofert, uczniów, płatności oraz komunikacji. Celem Tuvio było połączenie tych procesów w jeden spójny system, w którym zarówno uczeń, jak i korepetytor mogą obsługiwać cały proces nauki z jednego miejsca.',
        challenge: 'Największym wyzwaniem było zaprojektowanie architektury obsługującej wiele powiązanych ze sobą procesów biznesowych, takich jak marketplace ofert, dostępność korepetytorów, rezerwacja lekcji, płatności, subskrypcje, opinie oraz komunikacja realtime. System musi zapewniać spójność danych pomiędzy tymi obszarami, jednocześnie pozostając łatwym do rozbudowy. Istotnym wymaganiem było również przygotowanie architektury, która początkowo może działać jako modularny monolit, ale w przyszłości pozwoli na wydzielenie najbardziej niezależnych komponentów, takich jak komunikacja.',
        solution: 'Backend Tuvio został podzielony na niezależne moduły domenowe odpowiedzialne za konkretne obszary systemu. API zostało zbudowane w Laravelu z wykorzystaniem Laravel Sanctum + Fortify. Marketplace odpowiada za profile korepetytorów, oferty, ceny i dostępność terminów, natomiast moduł lekcji obsługuje rezerwacje oraz realizację zajęć. System płatności został zaprojektowany jako warstwa integrująca wiele bramek płatniczych bez uzależniania logiki domenowej od konkretnego operatora. Komunikacja pomiędzy uczniami i korepetytorami wykorzystuje Laravel Broadcasting i WebSockety, dzięki czemu może być później wydzielona do niezależnego mikroserwisu. Frontend został przygotowany w Nuxt 4 i komunikuje się z backendem poprzez API.',
        decisions: [
          "Podział backendu na niezależne moduły domenowe zamiast tworzenia jednego dużego monolitycznego obszaru aplikacji",
          "Wykorzystanie UUID jako identyfikatorów encji w całym systemie",
          "Wykorzystanie Laravel Sanctum do obsługi uwierzytelniania aplikacji",
          "Oddzielenie logiki biznesowej od kontrolerów poprzez wyspecjalizowane serwisy",
          "Wykorzystanie API Resources do kontrolowania struktury odpowiedzi API",
          "Zaprojektowanie marketplace wokół profili korepetytorów, ofert, cenników oraz ich dostępności",
          "Centralizacja obsługi płatności z możliwością integracji wielu operatorów",
          "Wykorzystanie eventów, powiadomień i broadcastingu do obsługi funkcji realtime",
          "Przygotowanie komunikacji jako modułu, który może zostać w przyszłości wydzielony do osobnego mikroserwisu",
          "Rozdzielenie danych użytkownika od profili ucznia i korepetytora",
        ],
        responsibilities: [
          "Projektowanie architektury backendu oraz podziału systemu na moduły domenowe",
          "Implementacja API w Laravelu",
          "Projektowanie modeli, relacji oraz struktury danych systemu",
          "Implementacja marketplace korepetytorów oraz systemu ofert i cenników",
          "Implementacja mechanizmu dostępności korepetytorów i wykrywania konfliktów terminów",
          "Budowa systemu rezerwacji i zarządzania lekcjami",
          "Implementacja systemu opinii wystawianych przez uczniów po zakończonych lekcjach",
          "Projektowanie warstwy integracji płatności z wieloma operatorami",
          "Implementacja systemu powiadomień oraz komunikacji realtime",
          "Budowa API dla paneli ucznia i korepetytora",
          "Przygotowanie architektury umożliwiającej późniejsze wydzielenie wybranych modułów do mikroserwisów",
        ],
        infrastructure: [
          "Backend API oparty o Laravel jako modularny monolit",
          "Moduły domenowe odpowiedzialne m.in. za Marketplace, Lessons, Payments, Subscriptions, Notifications oraz Chat",
          "Laravel Sanctum odpowiedzialny za uwierzytelnianie użytkowników",
          "Laravel API Resources do standaryzacji odpowiedzi API",
          "Laravel Events, Notifications oraz Broadcasting do obsługi zdarzeń i funkcji realtime",
          "WebSockety wykorzystywane do komunikacji realtime pomiędzy użytkownikami",
          "Frontend oparty o Nuxt 4",
          "Integracja frontendu z API poprzez warstwę Sanctum",
        ],
        features: [
          "Profile uczniów i korepetytorów",
          "Publiczne profile korepetytorów dostępne poprzez unikalny username",
          "Marketplace ofert korepetytorów",
          "Definiowanie cen oraz parametrów lekcji",
          "System dostępności korepetytorów oparty o ich harmonogramy i zajęte terminy",
          "Rezerwowanie oraz zarządzanie lekcjami",
          "Zmiana terminów lekcji poprzez system propozycji reschedule",
          "Obsługa różnych trybów prowadzenia lekcji",
          "System płatności z możliwością integracji wielu operatorów",
          "Obsługa subskrypcji i faktur",
          "Czat pomiędzy uczniem i korepetytorem",
          "Wskaźnik pisania oraz komunikacja realtime",
          "Wystawianie opinii po każdej ukończonej lekcji",
          "Statystyki i dashboardy dla uczniów oraz korepetytorów",
          "Integracja z zewnętrznymi platformami do prowadzenia lekcji online",
        ],
        outcome: [
          "Powstanie kompletnej architektury backendu platformy edukacyjnej obejmującej najważniejsze procesy marketplace i prowadzenia lekcji",
          "Stworzenie modularnego API pozwalającego niezależnie rozwijać poszczególne obszary systemu",
          "Implementacja procesu od znalezienia korepetytora, poprzez rezerwację i płatność, aż po realizację lekcji i wystawienie opinii",
          "Przygotowanie infrastruktury realtime umożliwiającej komunikację pomiędzy użytkownikami",
          "Zaprojektowanie warstwy płatności niezależnej od konkretnego operatora",
          "Przygotowanie systemu do dalszego skalowania oraz stopniowego wydzielania wybranych modułów do niezależnych mikroserwisów",
        ],
      },
